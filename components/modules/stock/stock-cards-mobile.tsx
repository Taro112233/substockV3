...
export function StockCardsMobile({ 
  ...
  const categoryOptions = [
    ...
    { value: 'EXTEMP', label: 'ยาใช้ภายนอก' },
      ...
      <div className="space-y-4">
                  ...
                  <CardContent className="p-3">
                      ...
                      <div className="col-span-2 min-w-0">
                        ...
                        <div className="text-xs text-gray-600">
                          {stock.drug?.dosageForm && (
                            <span className="font-medium">{stock.drug.dosageForm}</span>
                          )}
                          {stock.drug?.strength && (
                            <span> • {stock.drug.strength} {stock.drug.unit || ''}</span>
                          )}
                        </div>
                      ...
                      <div className="flex flex-col items-end justify-between">
                        ...
                        <div className="text-right mb-2">
                          <div className="font-bold text-sm text-purple-600">
                            ฿{stockValue >= 1000 ? `${(stockValue/1000).toFixed(1)}K` : stockValue.toFixed(0)}
                          </div>
                          <div className="text-xs text-gray-500">
                            @฿{(stock.drug?.pricePerBox || 0).toFixed(0)}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-100">
                      <div className="flex items-center gap-1 text-xs">
                        <Clock className={`h-3 w-3 ${lastUpdatedColor}`} />
        ...
        {filteredStocks.length > 0 && (
          <div className="text-center text-xs text-gray-500 pb-2">
            <p>
              แสดง <strong>{filteredStocks.length}</strong> / <strong>{stocks.length}</strong> รายการ
            </p>
          </div>
        )}
